...
export default function TodoModal({
  ...
  useEffect(() => {
    ...
    if (initialTodo) {
      ...
      const [h, m] = (
        initialTodo.schedule?.time ??
        initialTodo.reminder?.time ??
        `${defaultTime.hour}:00`
      ).split(":");
      ...
      setScheduledDate(
        initialTodo.schedule?.date ??
          initialTodo.reminder?.date ??
          formatDateForInput(new Date())
      );
  ...
  const handleDeleteCategory = (targetCategory) => {
    ...
    const fallback =
      categories.find((item) => item !== targetCategory) ?? "その他";
                ...
                <div className="max-h-[220px] overflow-y-auto">
                  {categories
                    ...
                    .map((item) => (
                      <div
                        key={item}
                        className="flex h-12 items-center justify-between bg-white px-4 active:bg-slate-100"
                      >
                        <div
                          onPointerDown={(e) => {
                            e.preventDefault();
                ...
                <div className="border-t border-slate-100 bg-white">
                  {isAddingCategory ? (
                    <input
                      ...
                      value={newCategory}
                      onChange={(e) =>
                        setNewCategory(e.target.value.slice(0, 30))
                      }
                      onBlur={() => {
